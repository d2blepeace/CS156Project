...
MODEL_NAME = "NaiveBayes-Gaussian"

# Save into trained_models/
MODEL_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "trained_models", "naive_bayes.pkl")
)


def _fill_numeric_nans_train_test(X_train, X_test):
    """
    GaussianNB cannot handle NaN.
    Fill numeric NaNs using training-mean and apply same means to test.
    """
...
def train_model(X, y):
    """
    Train a Gaussian Naive Bayes model with an 80/20 train–test split.
...
def save_model(model, path=MODEL_PATH):
    """Save trained Naive Bayes model to disk."""
...
def load_model(path=MODEL_PATH):
    """Load a previously trained Naive Bayes model."""
